...
function ShopGrid() {
  ...
  useEffect(() => {
    ...
    API.get(`/front-products/${currentPageRoute}`)
      .then((response) => {
    ...
    API.get(`/category-filters-list/${currentPageRoute}`)
      .then((response) => {
        setFilterCategoryList(response?.data?.categories);
        setFilterColorValues(
          response?.data?.variations?.reduce(
            (acc, curr) =>
              acc.find((v) => v?.value_name === curr?.value_name)
                ? acc
                : [...acc, curr],
            []
          )
        );
        setFilterBrandValues(
          response?.data?.brands?.reduce(
            (acc, curr) =>
  ...
  function onSortByChange(e) {
    ...
    if (e.target.value == "low_to_high") {
      sortByProductList = products?.sort((el1, el2) =>
        el1?.productvariations?.upper_price?.localeCompare(
          el2?.productvariations?.upper_price,
          undefined,
          { numeric: true }
        )
      );
      ...
      sortByProductList = products?.sort((el1, el2) =>
        el2?.productvariations?.upper_price?.localeCompare(
          el1?.productvariations?.upper_price,
          undefined,
          { numeric: true }
        )
      );
      ...
      console.log(
        "in_stock :: ",
        products?.filter((v) => v?.productvariations?.in_stock === 1)
      );

      sortByProductList = products?.filter(
        (v) => v?.productvariations?.in_stock === 1
      );
      ...
      API.post(`/category-list-filteration`, formdata)
        .then((response) => {
          setProducts(
  ...
  return (
    <main className="main shop">
      <Helmet>
        ...
        <meta
          name="description"
          content={`${pageSEODetails?.meta_description}`}
        />
      </Helmet>
      <PageHeader
        title={pageTitle}
                                  ...
                                  item?.value_type !== "3" ? (
                                    ...
                                    >
                                      <a
                                        ...
                                        style={{
                                          backgroundImage: `url(${item?.value_type_variation})`,
                                        }}
                                        key={index}
                                        onClick={(e) => handelColor(e, item)}
                                      >
