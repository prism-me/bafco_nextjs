                        ...
                        <OwlCarousel adClass="owl-simple owl-light owl-nav-inside cols-1" options={ { dots: false, nav: true, responsive: { 992: { nav: true } } } }>
                            {
                                post.image.map( ( item, index ) =>
                                    <ALink href={ `/blogs/${post.slug}` } key={ index }>
                                        <div className="lazy-overlay"></div>
                        ...
                        isAuthor ?
                            <>
                                <span className="entry-author">
                                    by <ALink href={ `/blogs/${post.slug}` }>{ post.author }</ALink>
                                </span>
                                <span className="meta-separator">|</span>
                            </>
                            : ""
                    }
                    <ALink href={ `/blogs/${post.slug}` }>{ date.toLocaleDateString( 'en-US', options ) }</ALink>
                    <span className="meta-separator">|</span>
                    <ALink href={ `/blogs/${post.slug}` }>{ post.comments } Comments</ALink>
                </div>
                ...
                </div>
                {
                    isContent ?
                        <div className="entry-content">
                            <p>{ post.content }</p>
                            <ALink href={ `/blogs/${post.slug}` } className="read-more">Continue Reading</ALink>
                        </div>
                        : ""
                }
            </div>
